describe("Reserve visit to the specjalist - with using correct user", () => {
  ...
  it("Check if you are on patient panel", () => {
    cy.url().should("include", "/panel-pacjenta");
  });
  ...
  it("Find the specialist in your city", () => {
    const specjalist = cy.get(
      'input[placeholder="specjalizacja lub nazwisko"]'
    );
    specjalist.click();
    const specjalistFound = cy
    ...
    cy.wait(3000);
    const cityFound = cy
      .get('[data-test-id="location-dropdown"]')
      .contains("Toruń");
    cityFound.click();
    cy.wait(3000);
  ...
  it("Choose first specialist from the list", () => {
    const chooseButton = cy
      .get('span[class="mb-1 avatar no-background"]')
      .eq(0);
    chooseButton.click();
  });
  ...
  it("Choose no symptoms covid button", () => {
    const symptomsNo = cy
      .get('[data-test-id="covid-symptoms-no-radio-button"]')
      .should("have.value", "false");
    symptomsNo.click();
  });

  it("Accept rules", () => {
    const rulesAccept = cy
      .get('[data-test-id="form-rules-checkbox"]')
      .should("have.value", "form-rules");
    rulesAccept.click();
  });

  it("I agree", () => {
    const agree = cy
      .get('[data-test-id="form-data-processing-checkbox"]')
      .should("have.value", "form-data-processing");
    agree.click();
  });

  xit("Reserve visit", () => {
    const reserve = cy
      .get('[data-test-id="booking-next-step-button"]')
      .contains("Umów wizytę");
